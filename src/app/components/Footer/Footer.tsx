...
export default function Footer() {
    ...
    useGSAP(() => {
        ...
        const ctx = gsap.context(() => {
            ...
            setTimeout(() => {
                gsap.set("[data-gsap='footer-1'], [data-gsap='footer-2'], [data-gsap='footer-3']", {opacity: 0, scale: 1.05});
                trigger = ScrollTrigger.create({
                    trigger: "[data-gsap='footer-1']",
                    start: "top 70%",
                    end: "bottom 70%",
                    animation: gsap.fromTo("[data-gsap='footer-1'], [data-gsap='footer-2'], [data-gsap='footer-3']", {opacity: 0, scale: 1.1}, {opacity: 1, scale: 1,stagger: 0.1,duration:1, ease: "power2.out"}),
                })
            }, 100);
        },[])
        return () => ctx.revert();
    ...
    return (
        <div className="relative pt-[170px] md:pt-[70px] pb-[200px] md:pb-[140px] w-screen h-fit bg-[#FAF5EF] flex flex-col items-center justify-center gap-[8vw] md:gap-[60px]">
            ...
            <div className="flex flex-col md:flex-row items-center justify-center gap-[2vw]  md:gap-[40px] text-blue text-[4.5vw] md:text-sm font-progLight">
                <a className="tracking-[-1px] hover:opacity-50 transition-opacity duration-150 cursor-pointer">About</a>
                <a className="tracking-[-1px] hover:opacity-50 transition-opacity duration-150 cursor-pointer">Terms & Conditions</a>
                <a className="tracking-[-1px] hover:opacity-50 transition-opacity duration-150 cursor-pointer">Contact</a>
            </div>
